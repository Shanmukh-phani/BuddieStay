  ...
  try {
    ...
    if (query && query.trim()) {
      ...
      searchCriteria.$or = [
        ...
        { room_no: searchRegex },
        { 'buddie_id.buddie_name': searchRegex }, // Add search for buddie_name
      ];
    }

    const complaints = await Complaint.find(searchCriteria)
      .populate({ path: 'buddie_id', select: 'buddie_name' });
...
router.get('/search-payment', async (req, res) => {
  ...
  try {
    // Determine if query is a valid number
    ...
    // Add conditions based on the type of query
    if (isNumeric) {
      // Handle numeric queries (match amount exactly)
      conditions.$or.push({ amount: numericQuery });
    }

    if (!isNumeric) {
      // If it's not a valid number or date, apply regex to string fields
      conditions.$or.push(
        { amount: { $regex: query, $options: 'i' } }, // Do NOT apply regex to numbers
      );
    }

    const payments = await Payment.find(conditions).populate('hostel_id', 'hostel_name'); // Populate hostel info if necessary
...
// Route to get buddies who haven't paid for the current month
// router.get('/unpaid-payments/:hostelId', async (req, res) => {
//   const { hostelId } = req.params;

//   // Get the current month and year for filtering payments
...
//   try {
//     // Step 1: Find all buddies associated with the specified hostel
//     const buddiesInHostel = await Buddie.find({ hostel_id: hostelId }).select('_id');

//     // Step 2: Find buddies who have payments for the current month
//     const buddiesWithPayments = await Payment.find({
//       hostel_id: hostelId,
//       month: currentMonthYear,
//       status: { $in: ['pending', 'accepted'] }
//     }).select('buddie_id');

//     // Step 3: Extract the buddy IDs that already have payments for the month
//     const buddiesWithPaymentIds = buddiesWithPayments.map(payment => payment.buddie_id.toString());

//     // Step 4: Filter out buddies who have not paid for the current month
//     const unpaidBuddies = buddiesInHostel.filter(
//       buddy => !buddiesWithPaymentIds.includes(buddy._id.toString())
//     );

//     // Step 5: Populate unpaid buddies' details (optional)
//     const unpaidBuddiesDetails = await Buddie.find({ _id: { $in: unpaidBuddies.map(b => b._id) } });

//     res.json({
//       month: currentMonthYear,
//       unpaidBuddies: unpaidBuddiesDetails
//     });
//   } catch (error) {
...
// Utility function to get the last three months in "Month Year" format and as Date objects
const getLastThreeMonths = () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    dates.push(new Date(year, currentDate.getMonth(), 1)); // First day of each month
...
// Route to get unpaid buddies for the past three months, respecting their joining date
router.get('/unpaid-payments/:hostelId', async (req, res) => {
  const { hostelId } = req.params;
  const { months, dates } = getLastThreeMonths(); // This function provides the last three months in the format you need

  try {
    // Step 1: Find all buddies in the specified hostel, including joining date, name, phone, and room number
    const buddiesInHostel = await Buddie.find({ hostel_id: hostelId })
      .select('_id buddie_doj buddie_name buddie_contact room_no');

    // Step 2: Find buddies who have payments for the last three months
    const buddiesWithPayments = await Payment.find({
      hostel_id: hostelId,
      month: { $in: months },
      status: { $in: ['pending', 'accepted'] }
    }).select('buddie_id month');

    // Step 3: Map to track which buddies have paid for each month
    const buddiesWithPaymentsMap = {};
    buddiesWithPayments.forEach(payment => {
      const month = payment.month;
      if (!buddiesWithPaymentsMap[month]) buddiesWithPaymentsMap[month] = new Set();
      buddiesWithPaymentsMap[month].add(payment.buddie_id.toString());
    });

    // Step 4: Filter buddies to find those who joined before each month and have not paid
    const unpaidBuddies = [];
    buddiesInHostel.forEach(buddy => {
      const buddyIdStr = buddy._id.toString();
      months.forEach((month, idx) => {
        const joinDate = new Date(buddy.buddie_doj);
        const monthStartDate = dates[idx];

        // Check if buddy joined on or before the month and has not paid for it
        if (joinDate <= monthStartDate && (!buddiesWithPaymentsMap[month] || !buddiesWithPaymentsMap[month].has(buddyIdStr))) {
          unpaidBuddies.push({
            buddy_id: buddy._id,
            month,
